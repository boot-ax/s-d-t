...
'$q',
  ...
  function ($mdDialog, $scope, $mdEditDialog, $http,$mdToast) {
    ...
    $scope.resetPassword = function($user){
      ...
      controller: 'forgotVerfiyController',
      controllerAs: 'fgverifyCtrl',
      focusOnOpen: false,
      templateUrl: 'partials/forgot-verify.html',
      // scope: $scope,
      resolve: {
           response: function () {
             return $response;
  ...
  angular.module('SE_App').controller('forgotVerfiyController', ['$mdDialog','$scope', '$mdEditDialog', '$http','$mdToast','response',
  function ($mdDialog, $scope, $mdEditDialog, $http,$mdToast,response) {
    'use strict';
    this.cancel = function() {
      $mdDialog.cancel();
    };
    ...
    this.user.user_email = response.data;

    // this.testercom = function(){
    //   console.log('damn nilly');
    // }

    this.testercom = function (event,user){
    $http.post('/service/verify-out-email',{$profile: user}).then(function(response){
      $mdToast.show({
      hideDelay   : 4000,
      position    : 'top center',
      controller  : 'ToastCtrl',
