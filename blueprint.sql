...
CREATE TABLE `W2_accounts` (
  ...
  CONSTRAINT `user_to_reg` FOREIGN KEY (`user_ID`) REFERENCES `registration` (`user_ID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `w2-to-account` FOREIGN KEY (`account_ID`) REFERENCES `account` (`account_ID`) ON DELETE CASCADE ON UPDATE CASCADE
...
INSERT INTO `W2_accounts` VALUES ('directv.com','cycorson@gmail.com','mulchaTNA1313','directv.com',33,NULL,94,201111),('ertwer','ggwertw5','werggrewer','wert4',49,'werg',96,201112);
...
CREATE TABLE `account` (
  ...
  `flat_date` date DEFAULT NULL,
  PRIMARY KEY (`account_ID`),
  UNIQUE KEY `stripe_customer_ID_UNIQUE` (`stripe_customer_ID`)
) ENGINE=InnoDB AUTO_INCREMENT=201114 DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `account` VALUES (201110,'superuser','stripe1',NULL),(201111,'Search Explosion','stripe2',NULL),(201112,'tester','stripe3',NULL);
...
CREATE TABLE `domains` (
  `domain_name` varchar(252) NOT NULL DEFAULT '',
  ...
  CONSTRAINT `d_to_account` FOREIGN KEY (`account_ID`) REFERENCES `account` (`account_ID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `host-key` FOREIGN KEY (`hosting_ID`) REFERENCES `hosting` (`hosting_ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
CREATE TABLE `hosting` (
  `hosting_name` varchar(252) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT '',
  `login_url` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
  `username` varchar(382) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
  `password` varchar(382) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
...
INSERT INTO `hosting` VALUES ('ip Networx','cp.ip-networx.com','13thfletch@gmail.com','AH3-XFd-*Ya!','2017-01-06','2017-02-16',7787,'greensburgracquetclub.com',2,201111),('SeoHosting.com','https://customers.seohosting.com/clientarea.php','13thfletch','My2Dogs!','2013-05-25','2016-11-25',7787,'insulationphoenixaz.com',3,201111),('Digital Ocean','Need to Enter','fletch@searchexplosion.com','SmallBatch32@','2017-01-18','2017-01-18',0,NULL,7,201111),('nonameinternet.com','https://www.nonameinternet.com/portal/clientarea.php','fletch@searchexplosion.com','My2Dogs!','2017-01-25','2017-01-25',7787,NULL,9,201111),('Godaddy','godaddy.com','68759966','mulchaTNA1313','2017-02-22','2017-12-22',7787,'cinexs.com',15,201111),('testhost','testhost','testhose','testhost','2017-03-12','2017-03-12',0,NULL,16,201112),('thost','tttt','sa','ffefe','2017-03-14','2017-03-14',3333,'asdfsa',18,201112);
...
INSERT INTO `links` VALUES ('33111','331uuu','33','dd33','33','Yes','2017-01-28','33',7,201111);
...
CREATE TABLE `registrar` (
  ...
  CONSTRAINT `regis_to_reg` FOREIGN KEY (`user_ID`) REFERENCES `registration` (`user_ID`) ON DELETE CASCADE ON UPDATE CASCADE
...
INSERT INTO `registrar` VALUES ('BlueHost','https://my.bluehost.com/web-hosting/cplogin','greenbuildprofits.com','My2Dogs!',7787,31,4,201111),('eNom','https://www.enom.com/login.aspx','nj-whoboy13','cowboy25',7787,31,5,201111),('Godaddy','godaddy.com','68759966','mulchaTNA1313',7787,31,6,201111),('Namecheap','namecheap.com','13thfletch','SmallBatch32@',7787,31,7,201111),('test','test','test','test',1234,47,8,201112),('tts','t','e','e',1111,49,10,201112);
...
CREATE TABLE `registration` (
  ...
  `user_email` varchar(252) NOT NULL DEFAULT '',
...
INSERT INTO `registration` VALUES ('Joseph Kolnik','jkolnik@mac.com','9e69eb3a0f4e268ead6ac82a7fb7bb3e','8012311782','Phoenix','1',NULL,'superuser',201110,30,'22286a84f60bf3caa102056c7adecb5b','yes'),('Joshua Fletcher','fletch@searchexplosion.com','7dd59a8302ec5d2a6edd5cfcb703e58d','4806528898','Phoenix','1',NULL,'admin',201111,31,'3986842787322e2fa2eaaf41dc0b91aa','yes'),('Jordan Beriault','sdarrstudios@gmail.com','b606bc9430e9dbea35bb5e8376777814','4808629463','Phoenix','1',NULL,'admin',201111,32,'d3ee04fac55efa465b3c2c701c0ff846','yes'),('Cynthia Fletcher','cycorson@gmail.com','7dd59a8302ec5d2a6edd5cfcb703e58d','','','1',NULL,'user1',201111,33,'e250ce6146d3de1312254054878026f9','yes'),('David Mills','something@something','7dd59a8302ec5d2a6edd5cfcb703e58d','','Somewhere','1',NULL,'user2',201111,34,'e250ce6146d3de1312254054878026f9','yes'),('Phil Switzer','phil@bricktownseo.com','7dd59a8302ec5d2a6edd5cfcb703e58d','','Oklahoma','1',NULL,'user1',201111,35,'e250ce6146d3de1312254054878026f9','yes'),('Joseph Kolnik','joe@searchexplosion.com','3d3ad7da30f8c3f3e1532de2ee31da93','8012311782','Phoneix, AZ',NULL,NULL,'admin',201111,46,'6bf0b82db23976366d4d1eb966fee647','yes'),('tester','tester@tester.com','f5d1278e8109edd94e1e4197e04873b9','tester','tester',NULL,NULL,'admin',201112,47,'d88d09d9216a4958497f929dab03114c','yes'),('tester2','tester2@tester2.io','2e9fcf8e3df4d415c96bcf288d5ca4ba','tester2','tester2',NULL,NULL,'user1',201112,49,'1f4244d308cb3ead7e8cff7826321a84','yes'),('tester3','tester3@tester3.io','7effbf343c0f8ee164da2fe3ae01e8cb','tester3','tester3',NULL,NULL,'user3',201112,50,'05bee82ba6915732a621d84e7c89845b','yes');
...
CREATE TABLE `resource_login` (
  ...
  CONSTRAINT `resource_to_reg` FOREIGN KEY (`user_ID`) REFERENCES `registration` (`user_ID`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- Dump completed on 2017-03-16 17:14:01
